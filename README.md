...
* current flame sensor reading
* flame status indicators
* sensor cards (flame active, others disabled)
* an embedded ThingSpeak graph

### 3. **Flame Sensor Reading**
The function reads analog values from pin A0:
```cpp
...
Flame sensor is updated every **5 seconds**.
...
Every **15 seconds**, the NodeMCU sends flame sensor data to the ThingSpeak API:
```
...
|--------------------------|-----------------------------------------------|
| ESP8266 used | ✔️ |
| At least 1 sensor | ✔️ (Flame Sensor) |
...
* Temperature and light sensor cards are placeholders (disabled) but styled to show UI scalability.
