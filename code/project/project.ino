...
#define FLAME_PIN A0  // Flame sensor on analog pin
String writeAPIKey = "YOUR_WRITE_API_KEY_HERE";
...
void serveDashboard(WiFiClient &client) {
  ...
  // Temperature Sensor Card - DISABLED
  client.println("<div class='card disabled'>");
  client.println("<div>TEMPERATURE</div>");
  client.println("<div class='value temp'>NIL</div>");
  client.println("<div>CELSIUS</div>");
  client.println("</div>");

  // Flame Sensor Card - ACTIVE
  client.println("<div class='card active'>");
  client.println("<div>FLAME</div>");
  client.println("<div class='value flame'>" + String(currentFlameValue) + "</div>");
  client.println("<div>ANALOG</div>");

  if(currentFlameValue > 800) {
    client.println("<div style='color:#f28b82;margin-top:10px;font-size:0.9em;font-weight:600;'>DANGER</div>");
  } else if(currentFlameValue > 500) {
    client.println("<div style='color:#fdd663;margin-top:10px;font-size:0.9em;font-weight:600;'>WARNING</div>");
  } else if(currentFlameValue > 200) {
    client.println("<div style='color:#8ab4f8;margin-top:10px;font-size:0.9em;font-weight:600;'>DETECTED</div>");
  } else {
    client.println("<div style='color:#81c995;margin-top:10px;font-size:0.9em;font-weight:600;'>SAFE</div>");
  }

  client.println("</div>");

  // Light Sensor Card - DISABLED
  client.println("<div class='card disabled'>");
  ...
  client.println("<iframe src='https://thingspeak.com/channels/your_channel_id/charts/1?api_key=your_read_api&bgcolor=%232c2c3f&color=%23e0e0e0&width=700&height=300'></iframe>");
  client.println("</div>");

  client.println("<div class='status'>");
  client.println("UPDATED: " + String(millis()/1000) + "s AGO | AUTO-REFRESH: 7s");
  client.println("</div>");
  client.println("</div>");
...
void setup() {
  ...
  currentFlameValue = readFlame();
  Serial.println("Flame: " + String(currentFlameValue));
}
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED && millis() - lastPush >= pushInterval) {
    ...
    HTTPClient http;
    String url = "http://api.thingspeak.com/update?api_key=" + writeAPIKey + "&field1=" + String(currentFlameValue);
